...
function makeRow(overrides: Partial<RawRun["rows"][number]>): RawRun["rows"][number] {
  return {
    session_id: 1,
    worker_id: 0,
    iteration: 0,
...
describe("processRun", () => {
  ...
  it("pairs START/END events and computes time in seconds", () => {
    const result = processRun(makeRun({
      rows: [
  ...
  it("filters out PLAYBACK_* events", () => {
    const result = processRun(makeRun({
      rows: [
        makeRow({ event: "REQ_HOME_START", timestamp: 1000, input_line_number: 1 }),
        makeRow({ event: "REQ_HOME_END", timestamp: 1200, input_line_number: 1 }),
        makeRow({ event: "PLAYBACK_START", timestamp: 500, input_line_number: 0 }),
        makeRow({ event: "PLAYBACK_DONE", timestamp: 1300, input_line_number: 0 }),
      ],
    }))
    expect(result.paired).toHaveLength(1)
    expect(result.paired[0]!.event_base).toBe("REQ_HOME")
  ...
  it("increments concurrency on WS_OPEN_START and decrements on WS_CLOSE_END", () => {
    const result = processRun(makeRun({
      rows: [
        ...
        makeRow({ event: "REQ_GET_START", timestamp: 1700, input_line_number: 2 }),
        makeRow({ event: "REQ_GET_END", timestamp: 1800, input_line_number: 2 }),
      ],
    }))
  ...
  it("tags only sessions inside the overlap window as maintenance for multiple workers", () => {
    const result = processRun(makeRun({
      rows: [
        ...
        makeRow({ session_id: 10, worker_id: 0, event: "REQ_HOME_START", timestamp: 100, input_line_number: 1 }),
        makeRow({ session_id: 10, worker_id: 0, event: "REQ_HOME_END", timestamp: 200, input_line_number: 1 }),
        // Overlapping session on worker 0
        makeRow({ session_id: 11, worker_id: 0, event: "REQ_HOME_START", timestamp: 2000, input_line_number: 1 }),
        makeRow({ session_id: 11, worker_id: 0, event: "REQ_HOME_END", timestamp: 4000, input_line_number: 1 }),
    ...
    const mainIds = new Set(result.paired.filter(p => p.maintenance).map(p => p.session_id))
  ...
  it("falls back to all sessions as maintenance when no session fits the overlap window", () => {
    const result = processRun(makeRun({
      rows: [
        ...
        makeRow({ session_id: 2, worker_id: 1, event: "REQ_HOME_END", timestamp: 5100, input_line_number: 1 }),
      ],
    }))
    expect(result.paired.every(p => p.maintenance)).toBe(true)
  })
