...
export function renderSessionDuration(state: AppState): void {
  ...
  if (desc)
    desc.textContent =
      ...
      " maintenance session" +
      (nSess !== 1 ? "s" : "") +
      " ordered from fastest to slowest completion time. The red line marks how long the original recording session took to complete (~" +
      cutoffSec +
      "s). Sessions should end around the same time as each other for consistent behavior."
  ...
  const chart = Plot.plot({
    ...
    y: {
      label: "Sessions (ordered by total duration)",
      domain: yDomain,
      axis: null,
    },
    color: {
