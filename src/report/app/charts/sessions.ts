...
export function renderSessions(state: AppState): void {
  ...
  if (desc)
    desc.textContent =
      ...
      " simulated user" +
      (nWorkers !== 1 ? "s" : "") +
      " executing back-to-back sessions. Warmup or cooldown sessions (desaturated) start before or end after the vertical dotted line. Narrower event bars mean better performance."

  const maint = data.filter((d) => d.maintenance)
  const maintMin =
    maint.length > 0 ? (d3.min(maint, (d) => d.start) ?? null) : null
  const maintMax =
    maint.length > 0 ? (d3.max(maint, (d) => d.end) ?? null) : null
  ...
  const marks: Plot.Markish[] = [
    Plot.barX(data, {
      ...
      title: (d: (typeof data)[number]) =>
        state.getRecordingLabel(d.input_line_number) +
        "\n" +
        d.time.toFixed(2) +
        "s",
    }),
