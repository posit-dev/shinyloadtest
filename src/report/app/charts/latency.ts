...
function renderLatencyFaceted(
  ...
  for (let ri = 0; ri < state.runs.length; ri++) {
    ...
    const data = classifiedRunData(run).filter((d) =>
      eventLabels.includes(d.eventLabel),
    )
      ...
      for (const label of EVENT_ORDER) {
        ...
        if (evts.length > 0) {
          ...
          chartData.push({
            ...
            eventLabel: label,
            latency: total,
            maintenance: evts[0].maintenance,
          })
        }
      }
    }
    ...
    const maintSessions = [
      ...new Set(chartData.filter((d) => d.maintenance).map((d) => d.session)),
    ]
      ...
      Plot.barY(
        ...
        eventLabels.length > 1
          ? Plot.stackY({
              ...
              y: "latency",
              fill: "eventLabel",
              order: EVENT_ORDER as unknown as string[],
              title: (d: (typeof chartData)[number]) =>
    ...
    if (maintSessions.length > 0 && maintSessions.length < allSessions.length) {
      const maintIndices = maintSessions
        .map((s) => allSessions.indexOf(s))
        .filter((i) => i >= 0)
      ...
      if (maxIdx < allSessions.length - 1)
        boundaryLabels.push(allSessions[maxIdx])
      if (boundaryLabels.length > 0) {
        marks.push(
          Plot.ruleX(boundaryLabels, {
    ...
    const chart = Plot.plot({
      ...
      color: {
        domain: EVENT_ORDER as unknown as string[],
        range: EVENT_ORDER.map((e) => EVENT_COLORS[e]),
      },
      marks,
    })
